...
> {
  ...
  async executeImplementation(
    parameters: WeatherFunctionParameters,
  ): Promise<ChatCompletionFunctionExecutionResult<WeatherFunctionResult>> {
...
const weatherFunctionInstanceMock = new WeatherFunction(
  (location: string, unit: string) =>
    Promise.resolve({ temperature: 20, description: 'Sunny' }),
);

export {
  weatherFunctionInstanceMock,
  WeatherFunction,
