...
describe('SplitLongFileFunction', () => {
  ...
  afterAll(async () => {
    ...
    for (let i = 0; i < 5; i++) {  // 5 chunks
      await fsPromises.unlink(`${testDirectoryPath}/chunk_${i}.txt`).catch(() => {});
    }
  ...
  test('splitting a non-existing file fails', async () => {
    const result = await splitLongFileFunction.execute(
      JSON.stringify({ sourceFileFullPath: 'non_existing_file.txt', maxTokens: 1000, overlap: 100, firstChunkDirectory: testDirectoryPath }),
    );

    expect((result as ChatCompletionFunctionExecutionResult<SplitLongFileResult>).content[0].startsWith('Error:')).toBe(true);
  });

  test('splitting a long file works', async () => {
    const result = await splitLongFileFunction.execute(
      JSON.stringify({ sourceFileFullPath: testFilePath, maxTokens: 1000, overlap: 100, firstChunkDirectory: testDirectoryPath }),
    );
