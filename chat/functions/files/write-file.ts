...
export class WriteToFileFunction extends ChatCompletionFunctionBase<
  WriteFileFunctionParameters,
  WriteFileFunctionResult
> {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  protected executeImplementation(
    parameters: WriteFileFunctionParameters,
  ): Promise<ChatCompletionFunctionExecutionResult<WriteFileFunctionResult>> {
    let flag = 'w';
  ...
  public description =
    'writes a content provided under "content" property to a file with a name provided under "fileName" property. The "writeCondition" property specifies under what conditions write operation will fail. Function returns "File saved" if operation was successful or message starting with "Error: " if operation failed';
