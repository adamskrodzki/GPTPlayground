...
describe('WeatherFunction', () => {
  ...
  beforeEach(() => {
    weatherFunction = new WeatherFunction((location: string, unit: string) =>
      Promise.resolve({ temperature: 20, description: 'Sunny' }),
    );
  });

  test('should have correct name and description', () => {
    expect(weatherFunction.name).toBe('get_current_weather');
    expect(weatherFunction.description).toBe(
      ...
      parameters: {
        ...
        properties: {
          ...
          unit: {
            type: 'string',
            description:
              'The unit of temperature, enum of celsius or fahrenheit',
  ...
  test('execute should return correct weather', async () => {
    const weather = await weatherFunction.execute(
