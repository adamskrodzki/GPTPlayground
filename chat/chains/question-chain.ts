...
function ensureAIThoughtsFunction(functions: IChatCompletionFunction[], chat: OpenAIChat): void {
...
function ensureQuestionStopFunction(functions: IChatCompletionFunction[], chat: OpenAIChat): void {
  if (functions.filter(x => x.name === 'question_stop_function').length === 0) {
    chat.addSupportedFunction(questionStopFunctionInstance);
  }
}

async function questionChain(question : string, debug = false) : Promise<{
  response: string;
  totalUsed: number;
}> {
  ...
  const systemMessage = 'As an intelligent assistant, your task is to use the functions available to you to answer question user have. \
  ...
  console.log('Loaded functions: ', chat.getSupportedFunctions().map(x => x.name));
  ...
  return new Promise<{
    ...
    questionStopFunctionInstance.beforeExecute = async (response) => {
      resolve({ response: response.finalMessage, totalUsed: chat.getTotalTokens() });
      isFinished = true;
      return response;
    };
