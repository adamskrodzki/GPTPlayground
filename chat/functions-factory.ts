...
class FunctionsFactory {
    ...
    public getFunctions(functionNames: string[]): IChatCompletionFunction[] {
        return functionNames.filter(x => this.functions[x]).map(x=>this.functions[x]);
    }

    public ListAvailableFunctions(): string {
        return Object.keys(this.functions).map(x => `${this.functions[x].name} - ${this.functions[x].description}`).join('\r\n');
    }
}
