...
async function getHierarchicalHeadersContent(url: string): Promise<NullableSection[]> {
    const response = await axios.get(url);
    const $ = cheerio.load(response.data);
  
    const sections: NullableSection[] = [];
    const currentSections: NullableSection[] = [null, null, null, null, null, null];
